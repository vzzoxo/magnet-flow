...
class RcloneClient {
  ...
  /** Storage quota of a remote: { total, used, free, ... } (may be unsupported). */
  about(remote) {
    return this.call('operations/about', { fs: `${remote}:` });
  }

  /** Dump the local rclone config (no API call) — used to read remote types. */
  configDump() {
  ...
  mkdir(remote, dir) {
    return this.call('operations/mkdir', { fs: `${remote}:`, remote: dir });
  }

  /** Delete a single file on a remote. */
  deleteFile(remote, file) {
    return this.call('operations/deletefile', { fs: `${remote}:`, remote: file });
  }

  /** Recursively delete a directory on a remote. */
  purge(remote, dir) {
    return this.call('operations/purge', { fs: `${remote}:`, remote: dir });
  }
}
