...
let cachedActiveDownloadNames = new Set();
...
async function collectDownloads() {
  ...
  // Update cached active download names
  const newNames = new Set();
  const allTasks = [
    ...out.active,
    ...out.waiting,
    ...out.stopped.filter(t => t.status !== 'complete' && t.status !== 'removed')
  ];
    ...
    if (t.files) {
      for (const f of t.files) {
        if (f.path) {
          ...
          if (firstSegment) {
            newNames.add(firstSegment);
          }
        }
      }
    }
  }
  cachedActiveDownloadNames = newNames;
...
function cleanupDownloadFiles(files, downloadDir) {
  ...
  for (const f of files) {
    ...
    let relPath = f.path;
    if (path.isAbsolute(relPath)) {
      relPath = path.relative(downloadDir, relPath);
    }
    const firstSegment = relPath.split(path.sep)[0];
    if (firstSegment) {
  ...
  // Delete top-level control files (.aria2) for torrents or files
  for (const entry of topLevelEntries) {
    const topAria2File = path.resolve(downloadDir, entry + '.aria2');
    try {
      if (fs.existsSync(topAria2File)) {
        fs.rmSync(topAria2File, { force: true });
      }
    } catch (e) {
      console.error(`[MagnetFlow] Failed to delete top-level control file ${topAria2File}:`, e.message);
    }
  }
...
function getActiveDownloadNames() {
  return cachedActiveDownloadNames;
}
