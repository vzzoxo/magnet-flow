...
// Bind address. Default 0.0.0.0 (all interfaces). Set HOST=127.0.0.1 when
// running behind a reverse proxy so the app is not reachable directly.
...
// Auto-upload finished downloads to a cloud remote (rclone). Empty = off.
const AUTO_UPLOAD_REMOTE = process.env.AUTO_UPLOAD_REMOTE || '';
const AUTO_UPLOAD_DEST = process.env.AUTO_UPLOAD_DEST || '';
...
// ── Transmission (optional second download engine) ──────────────────────────
const TRANSMISSION_RPC_URL = process.env.TRANSMISSION_RPC_URL || 'http://127.0.0.1:9091/transmission/rpc';
const transmission = new TransmissionClient(TRANSMISSION_RPC_URL);
