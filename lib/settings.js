...
function defaults() {
  return {
    ...
    // Default remote pre-selected in the manual "upload to cloud" dialog.
    defaultRemote: process.env.DEFAULT_REMOTE || '',
  };
}
...
function merge(base, patch) {
  const out = {
    autoUpload: { ...base.autoUpload },
    notify: { ...base.notify },
    aria2: { ...base.aria2 },
    defaultRemote: base.defaultRemote || '',
  };
  ...
  if (patch && typeof patch.defaultRemote === 'string') out.defaultRemote = patch.defaultRemote;
  return out;
}
...
const getDefaultRemote = () => state.defaultRemote || '';

module.exports = { init, update, getAll, getAutoUpload, getNotify, getAria2, getDefaultRemote, FILE };
